...
- **持久授权**（进行中流程的落盘授权凭证，供 resume 端双源核验；没有就写（无））:
  ...
  - 凭证: <文件绝对路径> 中 <授权字段> == <期望值>（例：`<WT>/.pr-review-loop/pr-14.json` 中 `mode` == `auto`）
  - 身份: <同一凭证文件里标识流程实例的字段 == 值>（例：`pr` == `14`、`worktree` == `<WT>`；把凭证钉死在「流程」那一个实例上）
  - 重入方式: <一句话，例：重跑 /pr-review-loop 续跑该 PR>
  - 授权范围: <免问的动作上限，到哪一步为止>（例：评审循环含合并 + cleanup 全程；或：到合并闸为止，合并另行确认）
...
5. **不要省略章节** —— 空就写 `（无）`。
6. **持久授权只认落盘凭证** —— 第 9 节该字段只能记录此刻真实存在的机器可读文件（路径 + 授权字段 + 期望值），且须由流程自身的 opt-in 契约写入。用户在对话里说过"行、继续"不算——那是会话级授权，随会话结束失效，resume 端会重新问。给不出路径就写 `（无）`。
7. **凭证必须自证身份** —— 同一凭证文件里必须有标识流程实例的字段（如 `pr`、`worktree`、flow id），把它们如实抄进「身份」子项，且须与「流程」「重入方式」指向同一实例。文件里没有任何身份字段 → 凭证不合格，整个字段写 `（无）`（resume 端会按无授权走确认门，安全降级）。
8. **授权范围如实、只窄不宽** —— 上限是凭证契约实际授予的动作边界；用户在会话里明确收窄过的（如"合并前问我"）**必须照录**——收窄记载即生效，它只会更安全；扩权仍只认落盘凭证。resume 端把该子项当免问上限，缺失则整份记载核验不过。
