...
唯一的例外是**持久授权（双源核验）**：handoff 第 9 节记载了进行中流程的落盘授权凭证（文件路径 + 授权字段 + 期望值 + 实例身份字段），且 resume 时实际读盘、值与身份全部仍相符——这说明用户对该流程的方向授权是以持久形式落过盘的，agent 照常复述，然后一行告知后直接续跑该流程，不再空等。两源缺一不可：handoff 没记载不放行（agent 不得自己翻 state 文件发明授权），盘上不符也不放行（中途切回手动、流程已结束等都被兜住）。身份字段把凭证钉死在声明的那一个流程实例上——借用别的实例的凭证（值恰好相同、但属于别的 PR / 别的 worktree）同样不放行。记载的「授权范围」则是续跑免问的上限：范围外的动作（例如记载写明合并需另行确认）仍会停下来问你，范围子项缺失则整份记载不生效。上个会话里口头说的"批准"永远不算持久授权——它随会话结束失效，这正是上一段的规则。
