...
唯一的例外是**持久授权（双源核验）**：handoff 第 9 节记载了进行中流程的落盘授权凭证（文件路径 + 字段 + 期望值），且 resume 时实际读盘、值仍相符——这说明用户对该流程的方向授权是以持久形式落过盘的，agent 照常复述，然后一行告知后直接续跑该流程，不再空等。两源缺一不可：handoff 没记载不放行（agent 不得自己翻 state 文件发明授权），盘上不符也不放行（中途切回手动、流程已结束等都被兜住）。上个会话里口头说的"批准"永远不算持久授权——它随会话结束失效，这正是上一段的规则。
