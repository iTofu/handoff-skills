...
The one exception is **durable authorization (two-source verification)**: if section 9 of the handoff records an on-disk authorization credential for an in-progress flow (file path + authorization field + expected value + instance-identity fields), and the resuming agent actually reads that file and every recorded field — value and identity — still matches, then the user's direction for that flow was durably approved — the agent still summarizes, then announces in one line and continues that flow instead of waiting. Both sources are required: no record in the handoff means no authorization (the agent may not dig up a state file on its own), and a mismatch on disk means no authorization either (covers mode switched back to manual, flow already finished, etc.). The identity fields pin the credential to the one flow instance named in the handoff — a credential borrowed from a different flow instance (whose value happens to match but belongs to another PR or worktree) does not qualify either. The recorded authorization scope is the ceiling of what may proceed unattended: actions beyond it (say, the record states that merging needs separate confirmation) still stop and ask, and a record missing its scope fails verification entirely. A verbal "go ahead" from the previous conversation never qualifies — that approval dies with the session, which is exactly the rule above.
